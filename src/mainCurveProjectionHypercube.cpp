...
void
runCurveProjectionLsh(std::string &iFileName, std::string &qFileName, std::string &outFile, int k = 4, int k_hi = 4, int w = 6000, int m = 0, int probes = 0, int M = 0, double e = 0.5) {
    /**
    ...
    ReduceTrajectories<CX,CY,X>(iDataList,iLabelList,testIDataList, testILabelList, dimension, iDataList.size()/1000);
    ...
    ProjectionCurveHQStruct * projHC = new ProjectionCurveHQStruct(dimension, dim, w, k, M, probes, DBL_MAX,e, "euclidean");
...
//    exit(1);
    ...
    results = unrollResult<DY, DX, CY> (ENNResults, ANNResults, testQLabelList);       // unroll the results and the stats to tuple strings.
    ...
    string res;
    res = "HC, 0," + getDatetime(false) + "," + to_string(probes) + "," + to_string(w) + "," +
...
//    runCurveProjectionLsh(std::string &iFileName, std::string &qFileName, std::string &outFile, int k_hypercube = 4, int k_hi = 4, int w = 6000, int m = 0, int probes = 0, int M = 0) {
